...
def create_sample_workbook(currencies: list = None,
                           entity_lei: str = "YOUR_LEI_HERE",
                           ref_date: str = "2025-12-31",
                           base_currency: str = "EUR") -> io.BytesIO:
    """
    Create sample EBA workbook with sheets per template × currency.
    Sheet naming: "C 72.00" (EUR), "C 72.00 USD", "C 72.00 GBP", …

    For base currency: only .a/.b sheets (is_ccy_sheet=False)
    ...
    for currency in currencies:
        ...
        for key, tdef in LCR_TEMPLATES.items():
            ...
            # base currency → only .a/.b sheets; foreign → only .w/.y sheets
            ...
            # Skip duplicates (e.g. C 76.00 .a and .b — keep first)
            if sheet_title in wb.sheetnames:
                continue

            ws = wb.create_sheet(title=sheet_title)
            _build_sheet(ws, tdef, currency, scale, seed, entity_lei, ref_date)
...
def _build_sheet(ws, tdef, currency, scale, seed,
                 entity_lei="YOUR_LEI_HERE", ref_date="2025-12-31"):
    random.seed(seed)
    ...
    # Rows 2-4: metadata (uses sidebar params)
    ws["A2"] = "Entity Name:"; ws["B2"] = "SAMPLE BANK SA"
    ws["D2"] = "Reference Date:"; ws["E2"] = ref_date
    ws["A3"] = "LEI:";           ws["B3"] = entity_lei
    ...
    for rid, rdef in rows.items():
        ...
        ic.value = rid.upper()
        ic.font = Font(bold=is_total or is_section, name="Arial", size=8)
        ic.alignment = CA; ic.fill = fill; ic.border = TB

        dc = ws.cell(row=er, column=2)
        dc.value = rdef["label"]
        dc.font = Font(bold=is_total or is_section, italic=is_section,
                       name="Arial", size=9)
